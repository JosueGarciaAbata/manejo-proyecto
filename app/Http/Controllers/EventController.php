...
class EventController extends Controller
{
    ...
    public function createEvent(Request $request)
    {
        $request->validate(
            ...
            [
                ...
                'event_end_time.after' => 'La hora de fin debe ser posterior a la hora de inicio.',
    ...
    public function editEvent(Request $request)
    {
        if (!request()->id_eve) {
            ...
            $event = Event::find(request()->id_eve);
            $data = [
                'event' => $event,
                'pageTitle' => 'Editar Evento',
            ];
            return view('back.pages.edit-event', $data);
        }
    }
    ...
    public function updateEvent(Request $request)
    {
        ...
        $request->validate(
            [
                ...
                'tit_eve' => 'required|unique:events,tit_eve,' . $request->id_eve . ',id_eve',
                'des_eve' => 'required',
                'fec_ini_eve' => 'required|date',
                'event_start_time' => 'required|date_format:H:i:s',
                'fec_fin_eve' => 'required|date|after_or_equal:fec_ini_eve',
                'event_end_time' => 'required|date_format:H:i:s|after:event_start_time',
                'featured_image' => 'mimes:jpeg,jpg,png|max:4096',
                'tag_eve' => 'required|max:50',
                'dir_eve' => 'required|max:255',
            ],
            [
        ...
        try {
            $event = Event::findOrFail($request->id_eve);
            ...
            if ($request->hasFile('featured_image')) {
                ...
                // Actualizar las rutas de imágenes en el evento
                $event->pre_img = 'thumbnails/thumb_' . $new_filename;
                $event->res_img = 'thumbnails/resized_' . $new_filename;
            }
            ...
            // Guardar los cambios
            if ($event->save()) {
                return response()->json(['code' => 1, 'msg' => 'Evento actualizado exitosamente.']);
            } else {
                return response()->json(['code' => 3, 'msg' => 'Error al actualizar el evento.']);
            }
        } catch (\Exception $e) {
            return response()->json([
                'code' => 3,
                'msg' => 'Algo salió mal: ' . $e->getMessage(),
                'trace' => $e->getTraceAsString() // Rastreo de errores
            ]);
        }
    }
}