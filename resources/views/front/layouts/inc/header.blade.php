    ...
    <div class="container">
        <nav class="navbar navbar-expand-lg navbar-light px-0">
            <a class="navbar-brand order-1 py-0" href="/">
                <img loading="prelaod" decoding="async" class="img-fluid" src="{{ asset(blogInfo()->blog_favicon) }}"
        ...
        $q->whereHas('posts');
                ...
                @if (Auth::check())
                    <li class="nav-item"> <a class="nav-link" href="{{ route('author.home') }}">Personal</a>
                    @else
                    <li class="nav-item"><a class="nav-link" href="{{ route('author.login') }}">Log in</a></li>
                @endif
            </div>
        </nav>
    </div>
